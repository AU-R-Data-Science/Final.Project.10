...
Use the function `logistic.regression()` to fit the model. Note that the parameter `Y` must be factors. `logistic.regression()` is a summary of analysis, the output of this function is a list which includes:

1.  The level of response;

2.  Estimated \Beta value;

3.  Initial \Beta value;

4.  The Confidence interval of \Beta;

5.  Confusion Matrix;

6.  Default metrics(with 0.5 cut-off value);

7.  Table of different cut-off and metrics.
...
The bootstrap process was performed when we called `logistic.regression()`.
And the confidence interval can also be calculated by `boot.confi()`.
```{r}
